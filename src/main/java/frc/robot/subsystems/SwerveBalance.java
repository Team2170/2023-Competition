...
public class SwerveBalance {
   ...
   * @param scalePow Weight the result to be nonlinear (faster to balance when
   *                 farther away). Set to
  ...
  /**
   * Update robot speeds for balancing based on pitch and roll. Creates a plane
   * which the robot sits
  ...
  public Translation2d calculate(Rotation3d angle) {
    ...
    var vyMetersPerSecond = BetterMath.signedAbsFunc(yGrad, (x) -> Math.pow(Math.abs(x * scale), scalePow));
    var vxMetersPerSecond = BetterMath.signedAbsFunc(xGrad, (x) -> Math.pow(Math.abs(x * scale), scalePow));
